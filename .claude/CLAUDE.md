...
- **Current design (spec revision 3): one mini PC runs both WorkPrinters (same room). Per machine: USB camera + USB "control box" (Raspberry Pi Pico 2 + relays) reading the RCA sync socket with µs timestamps, driving lamp/motor relays, camera trigger, heartbeat safety stop.** Pi-5-per-machine (rev 2) and Windows (rev 1) designs dropped. App: Python 3.12, FastAPI, V4L2, FFmpeg on Ubuntu 24.04. Firmware: C, Pico SDK. Syncmouse (owner's is PS/2) not needed; kept as no-wiring option via active PS/2-to-USB converter.
- Spec revision 3 awaiting owner review. Next: implementation plan (writing-plans skill).
- Open questions: XP model (photos of switches/inside back), optics, timing disk position, camera purchase, mini PC model (needs 2× USB 3 on separate controllers), license (MIT recommended).
...
- Everything is Linux (dev server, mini PC); develop and test against a simulated camera and simulated control board here.
